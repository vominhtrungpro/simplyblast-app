...
def create_user(content_frame,login,app):
...
class DataTable(tk.Frame):
    ...
    def create_table(self):
        headings = ["Name", "Email", "Role", "Tenant"]
        ...
        for row_index, row_data in enumerate(self.data, start=1):
            ...
            label1 = tk.Label(self, text=full_name, relief=tk.RIDGE, width=25)
            label1.grid(row=row_index, column=0, sticky="nsew")

            email = user.get("email", "")
            label2 = tk.Label(self, text=email, relief=tk.RIDGE, width=25)
            label2.grid(row=row_index, column=1, sticky="nsew")

            role = row_data.get("role", [])
            role_value = role[0] if role else "" 
            label3 = tk.Label(self, text=role_value, relief=tk.RIDGE, width=25)
            label3.grid(row=row_index, column=2, sticky="nsew") 

            tenant = row_data.get("tenantDto", [{}])[0]  
            billing_company_name  = tenant.get("billingCompanyName", "")
            waba_number  = tenant.get("wabaNumber", "")
            if waba_number:
                full_tenant = f"{billing_company_name} ({waba_number})"
            else:
                full_tenant = billing_company_name

            label4 = tk.Label(self, text=full_tenant, relief=tk.RIDGE, width=25)
            label4.grid(row=row_index, column=3, sticky="nsew")
...
def search(page_index,page_size,sort_field_name,sort_field_status,filter_name):
    url = auth_url + '/user/search-with-tenant'
    access_token = ''
    ...
    if isSuccess == True:
        ...
        messagebox.showerror("Lỗi", "Search user")
